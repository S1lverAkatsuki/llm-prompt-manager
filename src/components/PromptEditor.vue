...
const {
  tagInput, editingPrompt, isEditorInCreateMode, selectedTags,
  isEmptyTag, canAddTag, handleAddTag, handleRemoveTag, openEditor, resetEditor,
  canSave, handleSave, handleDelete, deletedTimeout, editorContextInputRef,
  editAreaRef, handleInputExpanded, isEmptyTitle, isEmptyContent
} = useEditor(items);
...
<template>
  <dialog v-if="editingPrompt" ref="dialogRef" id="editor" class="modal" @close="onDialogClose">
    <div class="modal-box w-[80vw] max-w-3xl h-[90vh] p-0 flex flex-col">
      <div class="flex items-center justify-between border-b border-base-300 px-6 py-4">
        <div>
          <h3 class="text-lg font-semibold text-base-content">{{ isEditorInCreateMode ? "添加新的 Prompt" : "编辑 Prompt" }}</h3>
      ...
      <div class="bg-base-200/50 border-t border-base-300 p-6 flex justify-end gap-2">
        <button v-show="!isEditorInCreateMode" class="btn btn-outline btn-error mr-auto" @click="onDelete">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-4 h-4" fill="currentColor">
